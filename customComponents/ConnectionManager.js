...
class ConnectionManager {
    ...
    constructor(relativeBaseFolder, usernameToken) {
        this.relativeBaseFolder = relativeBaseFolder;
        this.usernameToken = usernameToken;
    }

    generateConnBundle(username, ids, downloadPath, filename, callback) {
        if (ids.length === 1)
            this.generateJSON(username, ids, filename, callback);
        else
            this.generateBundle(username, ids, downloadPath, filename, callback)
    }

    generateJSON(username, ids, filename, callback) {
        let filter = {
            username: username,
            property: {$in: ids}
        };
        ...
        Connection.find(filter).select({
            ...
            callback({type:'JSON', content:components[0]});
        });
    }

    generateBundle(username, ids, downloadPath, filename, callback) {
        ...
        Connection.find(filter, 'property name icon iconUnicode structure translatedStructure').exec((err, components) => {
            let libs = [];
            let previews = [];
    ...
    loadConnection(schemaPath, username, callback) {
