...
class DataStoreMapper {
    ...
    exportDFD() {
        ...
        Object.values(this.dataFlowDiagram.connections).map(function (conn) {
            grammar.connections.push({
                start: conn.start, end: conn.end, type: conn.type, startIndex: conn.startIndex,
    ...
    importDFD(grammarJSON) {
        ...
        try {
            ...
            grammar.cards = grammar.cards.filter(function (card) {
                return vizParams.cards[card.type] !== undefined;
            });

            grammar.connections = grammar.connections.filter(function (conn) {
                return ConnectionTypes.property[conn.type] !== undefined;
            });

            let connIds = grammar.cards.map(function (card) {
                return card.id;
            });

            grammar.connections = grammar.connections.filter(function (conn) {
                return connIds.indexOf(conn.start) !== -1 && connIds.indexOf(conn.end) !== -1;
            });
